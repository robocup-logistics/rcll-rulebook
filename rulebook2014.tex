...
GmbH \& Co. KG with certain freedoms and limitations. This includes the current version, Robotino 3, as well as the faced out Robotino 2 until decided otherwise by the TC. See
...
The origin and coordinate system of the competition field is drawn in Figure \ref{fig:competition-area} and will be intrinsically referred to for each statement within this rulebook that uses absolute coordinates. All indicated sizes of mark-ups are to be considered outside
dimensions. The default width for mark-ups is \SI{19}{\milli\metre},
the default color is black, except for the recycling machines, in which case the machine space is marked with green mark-ups.
...
The robot insertion areas are located along each corner of the left competition area, with a separate mark-up line \SI{0.4}{\metre} apart from the competition boundary.
Next to them towards the mid, the input storage areas are located. Each input storage area has a size of \SI{1.1 x 0.4}{\metre}, is
painted blue on the wall, and contains a set of 22 pucks.
...
reside in the center of a squared machine space spanned by mark-ups with \SI{0.5}{\metre} each side.
...
The center coordinates and alignment of the machine and the different delivery gate slots are presented in Table~\ref{tab:coordinates}. Note that this field layout should be seen as a proposal for the actual competition area layout, accounting for symmetry and avoiding clustering of machine access nodes and unapproachable machines. However, in case of comprehensible reasons (e.g. hardware or exhibition issues), the field layout can still change before the actual tournament starts. Thus, teams should focus on a generic approach for production, allowing for easy adaptation of machine positions and alignments.
...
Appendix~\ref{apx:sec:engref} for further details. Machines can be in
...
in Table~\ref{tab:production-machines-function}.
...
cf.~\refsec{sec:out-of-order}). In case a data carrier is fed to a machine, which belongs to the opposing team, the red and the yellow LEDs are flashing at 2 Hz. The machine then consumes 
...
operating mode according to Tables~\ref{tab:production-machines-feedback}, \ref{tab:recycling-machine}, and \ref{tab:delivery-gates}.
...
The production portfolio is presented in Figure~\ref{fig:production-diagram}, which shows a production chain diagram with the machines and their respective inputs and outputs. The multistaged production processes can be repeated as long as enough pallet carriers can be provided to complete the cycle. The different machine types are specified in Section~\ref{sec:machines}.
...
pool containing all production machines. It will do so
at random points of time and with the same conditions for both teams, i.e., affecting the same machines for both teams. There will be 6 to 8 of such triggered
...
once per match. The recycling machines will be taken down exactly once per match, with a downtime of 20 to 40 seconds. If the machine turns offline during processing or
...
product. In each game, orders with a fixed delivery value of 150
points will be placed\footnote{The number can be adjusted during the
  tournament by the Technical Committee should it be reached reliably
  by multiple teams}. These points only include the points for the
final production step and delivery, not counting just-in-time
production, intermediate machine, or recycling points. Each order will
...
\begin{table}
  ...
  \subtable[Scoring scheme for the production phase]{\mytable{\begin{tabularx}{\linewidth}{p{7em}|X|p{2.4em}}
        ...
        its work cycle to the recycling machine& $+5$\\
				Obstruction Penalty & Deliver a data carrier to a machine, which belongs to the opposing team. & $-5$ \\
    \end{tabularx}  }}
...
Teams are penalized with -5 points for obstruction when delivering a data carrier to a machine of the opposing team. 
In this case, the puck becomes junk and cannot be recycled or used afterwards. If the robot leaves the machine space without the puck, the puck will be removed by a field referee.
...
\item If pushing occurs between a moving and a standing robot, the
  ...
  resolving it. If he is not able to do this, a pushing foul will be
  called.
...
A robot fleet consisting of three robots has to show that it can reach a certain goal location
avoiding collisions with static obstacles and other moving robots. Two teams compete
against each other at the same time. Team 1 places their robots inside the competition area with $x > 4.79$m (i.e., near
the delivery gate at the bottom), team 2 at $x < -4.79$m (i.e., near the delivery gate at
the top). All robots from each team must then reach the opposite
starting area faster than the opposing fleet. A robot reaches this area if at least half of the robot's volume crossed the border. For each robot not
...
other. Robot collisions will be handled the same as for the main challenge of the Logistics League, and are therefore punished accordingly. The team, which reaches
the respective goal location the fastest, wins. If no team reaches the goal
within three minutes, the robot fleet with the overall shortest distance
to the goal wins. Depending on the number of participating teams and certain time constraints, the tournament mode will be announced before the competition starts.

\paragraph{Approaching the MPS station~}
\begin{figure}[h]
	\includegraphics[width=.5\textwidth]{MPSDemo.jpg}
	\caption{Demo Scene (CIROS) of MPS approach}
	\label{apx:fig:mpstc}
\end{figure}
One robot will be required to use any gripping device to handle a workpiece. The challenge will be to
1. receive one workpiece from a shelf\\
2. approach a "Pick \& Place" station and hand over the workpiece\\
3. approach the backside of this station\\
4. wait to receive the completed workpiece\\
5. transport it to one empty shelf spot and unmount it.\\

Teams may add markers of any kind to the MPS station and shelf to support their approach. These markers have to be removable residue-free and mounted/unmounted in a reasonable short amount of time before each test run.\\

Each team has a ten minute window to conduct a test run with field exclusivity. Once they choose to begin, they have three minutes to complete the challenge.
Each step will be awarded with points. If all points are completed in time, the fastest time wins.\\

The installation will be available throughout the whole time of the tournament. Festo will provide one standardized gripper per team over the course of a tournament. Custom constructions are welcome.\\

\subsection{Details:}
Workpiece:
\begin{itemize}
\item Diameter(round): \SI{40}{\milli\metre}
\item Height: \SI{25}{\milli\metre}
\item Material: Plastic
\end{itemize}

\begin{figure}[h]
	\includegraphics[width=.5\textwidth]{selectionofworkpieces.jpg}
	\caption{Selection of MPS workpieces}
	\label{apx:fig:workpiece}
\end{figure}

MPS station:
\begin{itemize}
\item Profile plate dimension: \SI{350}{\milli\metre} $\times$ \SI{700}{\milli\metre}
\item plate height: \SI{30}{\milli\metre}
...
\item Height: \SI{148.165}{\milli\metre} minimum height.
\end{itemize}

\begin{figure}[h]
	\includegraphics[width=.5\textwidth]{mps_overview.png}
	\caption{Overview Pick \& Place}
	\label{apx:fig:mps overview}
\end{figure}

\begin{figure}[h]
	\includegraphics[width=.5\textwidth]{mps_docking.png}
	\caption{Interface point height}
	\label{apx:fig:mps dock}
\end{figure}
...
\subsection{The Mobile Robot System: Robotino 3 Premium:}
...
additional sensors or motors of devices. By default Power is supplied via two exchangeable
...
\subsubsection{Robot Dimensions (w/o extension tower)}\label{apx:sec:robot}
\begin{itemize}
...
\begin{itemize}
	\item[] \textbf{3x omnidrectional wheels:} \SI{120}{\milli\metre}
	\item[] \textbf{Fed by DC three motors:} \SI{3600}{rpm}
 	\item[] \textbf{Gear transmission ratio 32:1} \SI{22.5}{\kilogram}
 \end{itemize}

\begin{figure}[h]
\includegraphics[width=.5\textwidth]{Robotino3.jpg}
\caption{Robotino 3 without extension tower}
\label{apx:fig:Robotino3}
\end{figure}
...
The default webcam camera is plugged in via USB and is capable of full HD 1080p video with auto light correction and two built in microphones for stereo sound with noise-cancelling 

\subsubsection{Controller Board}

Robotino is powered by an exchangeable Embedded PC - COM Express layout combined with a custom made sensor board.

Embedded PC according to COM Express specification:
...
Embedded PC interfaces:
\begin{itemize}
\item 1 $\times$ Ethernet
\item 6 $\times$ USB 2.0
\item 2 $\times$ PCI Express expansion slot
\item Wireless Lan according to 802.11g and 802.11b specification in client or access point mode (exchangeable)
\item 2 $\times$  RS232
...
Motor control: 
\begin{itemize}
\item microcontroller with 32-bit microprocessor and separated Ethernet interface
\item Including 1 $\times$ additional motor output and encoder connector
\item 8 $\times$ analog inputs from 0 V to 10 V (50 Hz)
\item 8 $\times$ digital inputs/outputs with 24 V, short circuit proof and overload protected
\end{itemize}

\subsubsection{Power supply}
\begin{itemize}
\item 2 $\times$ 12 V lead-fleece rechargeable batteries with 9.5 Ah capacity each
...
Pre-installed is Ubuntu Linux 12.04 LTS operating system. The main part of the controller is the
...
applications via wifi. Also provided: API 2.0 with libraries which allow you
...
HTML5-baesd user interface provided by webserver running on Embedded PC for setup and configuration using SmartPhone, Tablet or PC/Notebook
User interface supporting program management, manual control, network setup, status display and options
Help system: online manual for getting started, details on all components and introduction into programming

\paragraph{Robotino View} 
For a quick start or Hardware testing there is proprietary drag and drop Software Robotino View.
Graphical programming environment for external PC running on Windows XP, Vista, Windows 7 or Windows 8.
...
Additional information as well as accessories can be obtained through
\url{http://www.festo-didactic.com/int-en/learning-systems/education-and-research-robots-robotino/scenarios-and-learning-areas.htm?fbid=aW50LmVuLjU1Ny4xNy4xOC44NTguNDc1NQ}

\subsection{Robotino 2.0 (Faceout):}
The mobile robot system Robotino is a platform with an open
mechanical interface for the integration of additional mechanical
devices and an open electrical interface to integrate easily
