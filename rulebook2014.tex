...
Finally, no rulebook is perfect. Teams should avoid searching for
rulebook gaps or inconsistencies to achieve advantages in the
competition. Instead they are explicitly requested to report such gaps
to the Technical Committee, so we can keep the necessity for rule
discussions during the RoboCup event to a minimum.
...
At the first day before the tournament begins, there is a short
meeting including the briefing of all named referees.

\subsection{Referee Delegation}
Each participating team of the tournament has to provide at least two
team members as referees. These referees should perform the following
criteria:
...
There are 3 referees for each competition: One refbox referee and two
field referees, which are each assigned to a particular field
half. One of the two field referees has the status of being the head
referee. The head referee has the upper hand when there is a referee
disagreement and then announces the final decision.
