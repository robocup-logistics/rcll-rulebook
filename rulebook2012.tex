...
Automated Guided Vehicles (AGV) in industrial applications. This includes current
challenges of developing industry-wide standards for Cyber Physical Production
Systems (CPPS) like designing plug-and-produce capable systems.
...
Our aim is to simulate autonomous guided vehicles (AGV) in industrial applications. 
...
LLSF main challenge is a multistaged production cycle of different product
variants with self-crafted intermediate products, as well as delivery of
...
different field slots are given in Table~\ref{tab:coordinates}.
...
light signals. Therefore all machines of the same type will indicate the same
light scheme. The light schemes are:
\begin{itemize}
...
\end{itemize}
The corresponding light signal to the five different machine types will be
induced by the referee box. Please consult the RoboCup Logistics League Referee
Box Specification for a detailled definition of the message type.
...
properly, they will not produce any junk.
...
\begin{table}[tp]
  ...
  \mytable{\begin{tabularx}{\linewidth}{p{0.35\linewidth}|X}
    ...
    Yellow LED flashing (at 2 Hz) & Finished product variant 2 (\p1{})\\%\hline
    Red LED flashing (at 2 Hz) & Finished product variant 3 (\p1{})\\%\hline
  \end{tabularx}}
