...
Currently, any \s0 can be used for the late order. However, in order to allow 
visually triggered processing as done for the former express good challenge, a \s0 
will be placed in the late order insertion slot above the input storage as soon as a late order is announced.
This \s0 puck will not be removed and can also be used for normal production. Note that, if the late order insertion slot already 
contains a \s0 when the next late order arrives, no further puck is inserted.

Furthermore, it is permitted to pre-produce a late-order, thus only the delivery of the late order has to be in time.

A robot may not be initially placed next to the insertion slot, but only in the robot insertion area (below the input storage).
