...
\begin{table}[tp]
  ...
  \mytable{\begin{tabularx}{\linewidth}{p{0.35\linewidth}|X}
    ...
    Red and green LED turned on & This gate is active, namely the
    designated gate.\\
    Red and organge LED turned on & False Delelivery to an active
    gate.\\
    Red LED turned on and Yellow LED flashing (at \SI{2}{\hertz}) &
    False delivery to inactive gate. The puck's state changes to consumed.
%    \hline
  \end{tabularx}}
...
be only one active gate at a time. False deliveries to an active and
inactive gate will be indicated as well according to
Table~\ref{tab:delivery-gates}. A pallet carrier which was delivered
and placed correctly in the delivery area right under the RFID reader
will be taken out by the referee after the delivery was indicated by
the respective light signal of the delivery gate. In particular, this
means that a correct and false delivery causes the pallet carrier to
be taken out by the referee. In case of a delivery to an active gate,
the team will be awarded with the points for the delivery; if the gate
was inactive, no points will be awarded. If, however, a pallet carrier
was not placed correctly under the RFID reader, the pallet carrier
will not be taken out by the referee and remain where it is.
...
and delivery of the requested product in time. 

Currently, any \s0 can be used for the late order. However, in order
to allow visually triggered processing as done for the former express
good challenge, an \s0 will be placed in the late order insertion slot
above the input storage as soon as a late order is announced.  This
\s0 puck will not be removed and can also be used for normal
production. Note that, if the late order insertion slot already
contains an \s0 when the next late order arrives, no further puck is
inserted.

Furthermore, it is permitted to pre-produce a late-order, thus only
the delivery of the late order has to be in time.
...
process.
The refbox will adjust the down times not to collide with late 
orders to allow production and delivery within the late order time.
...
\begin{itemize}
\item The active delivery gate will not switch during a match.
\item There will be two late order challenges. No late order challenge will be initialized during the first or within the last two match minutes.
\end{itemize} 
...
\begin{table}
  ...
  for 2 minutes.\\[1ex]
%
  Inactive robot & If a robot is inactive for 2 minutes, the referee
  can ban the robot from the field.\\[1ex]
