...
\item[Storage Station (SS)] provides 24 slots of storage divided into
  6 layers (\reffig{fig:SS_portrait}). Items can be shipped in for
  storing to the input side and shipped out on the output side on
...
\begin{figure}[tp]
    ...
    \caption{Blocked zones regarding the MPS rotation (red zones may
      not be used for other machines).}
...
be announced at the beginning of the tournament.\footnote{Please note
  ...
  future competitions.} The products provided from the SS are only
allowed to be used for $C_0$ orders with a quantity requested greater
than 1. Retrieving a base costs 10 points, which will be substracte
from the final score.\footnote{The TC is working towards the
