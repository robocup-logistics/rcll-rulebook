...
wall, and machines of types BS, DS, and SS are mirrored according to
...
slide of these stations are offset to sideways at the input side. This
...
and DS machines adjacent to a wall will always be placed such that the
...
faces south. Were this setup mirrored, the shelf on the cyan CS in
...
are blocked as well (fig.~\ref{fig:zone-mps-rotation-straight}).  For
machines with rotations \ang{45}, \ang{135}, \ang{225} and \ang{315}
three zones in front of each input and output side are blocked
(fig.~\ref{fig:zone-mps-rotation-odd}). No MPS may be placed in a
...
next to each other, a third machine will only be placed with an empty
zone spacing in-between.
