...
The robot insertion areas (gray area on \reffig{fig:competition-area}) are
outside of the main
...
these positions are differnt and will change from match to match.  Machines
have two kind of sides: active (wide, where the robot interacts) and
ignored (narrow) edges.
The constraints for positioning of the MPS are given in \refsec{sec:machine-swapping}.
...
\begin{table}[bh!]
  ...
  \begin{tabular}{|l|c|c|c|c|l|}
    ...
    &DS&81&\parbox[c]{1.5cm}{\includegraphics[width=1.2cm]{markers/figure_81}}&82&\parbox[c]{1.5cm}{\includegraphics[width=1.2cm]{markers/figure_82}}\\[2.5ex]
    &SS&193&\parbox[c]{1.5cm}{\includegraphics[width=1.2cm]{markers/figure_193}}&194&\parbox[c]{1.5cm}{\includegraphics[width=1.2cm]{markers/figure_194}}\\[2.5ex]\hline

    &&&&&\\[-2ex]
    \multirow{7}{*}{\rotatebox{90}{\bfseries Magenta\hspace{3.4cm}}}&CS 1&97&\parbox[c]{1.5cm}{\includegraphics[width=1.2cm]{markers/figure_97}}&98&\parbox[c]{1.5cm}{\includegraphics[width=1.2cm]{markers/figure_98}}\\[2.5ex]
    ...
    &DS&49&\parbox[c]{1.5cm}{\includegraphics[width=1.2cm]{markers/figure_49}}&50&\parbox[c]{1.5cm}{\includegraphics[width=1.2cm]{markers/figure_50}}\\[2.5ex]
    &SS&209&\parbox[c]{1.5cm}{\includegraphics[width=1.2cm]{markers/figure_209}}&210&\parbox[c]{1.5cm}{\includegraphics[width=1.2cm]{markers/figure_210}}\\[2.5ex]\hline
  \end{tabular}
...
The rotation of the MPS can have 8 possible angles, starting at \ang{0} with
\ang{45} steps. The orientation of the MPS Station is specified as follows. Each
MPS Station has a well defined local mathematical (x,y)-coordinate system: The x-axis is
given by the conveyor and the positive direction is in the direction from the
output to the input. The origin is just the center point of the conveyor. Hence the
orientation of a MPS Station is equal 0 if it's local coordinate system is in
alignment with the field coordinate system. Further the orientation is e.g. equal
\ang{90} if the local coordinate system is in alignment with the competition
field coordinate system  after a turning (counter clockwise) of \ang{90} of the
field coordinate system etc.

The orientations of the machines on the two halfs of the field are mirrored
along the y-axis. Thus, a machine on Magenta with an orientation of \ang{45}
(respectively \ang{135}, \ang{225}, \ang{315}) is related to a machine on Cyan
with an orientation of \ang{225} (respectively \ang{135}, \ang{225},
\ang{315}).
An exception from this rule is made for the CS and RS when place directly next to a wall.
Since the shelfs and slides should be reachable the MPS will be rotated that these sides will be facing towards the field center rather then the wall.
Therefore all CS and RS which are placed next to walls (zones: C-Z*1, C-Z*8, C-Z7*, M-Z*1, M-Z*8 and M-Z7*) are rotated in a way that the y-axis is facing away from the wall they are places next to.
...
Furthermore only two machines will be placed next to each other, a third machine will only be placed with an empty zone spacing in-between.
...
For consecutive games the changed zones for MPSes should be minimal for ease of the referees.
...
phase,\footnote{In 2017 there are no more fixed stations. The prefix ``C-'' means team
...
applies to all machine types BS, DS, CS, SS and RS.
When the DS, CS and RS
...
When the BS and SS received the setup the products are dispensed at the output and no timing requirement needs to be fulfilled.
...
second setup is performed the machine will
be temporarily broken (cf.~\refsec{sec:broken-machine}).

An MPS can be reset at any time by a dedicated msg (see referee box Integrator's Manual~\cite{RefBoxIntManual}).
Which cases the MPS to go into an broken state (cf.~\refsec{sec:broken-machine}).
...
randomized.
The $C_1$ order will be completely randomized.
...
However exploration points gained during the exploration will not be used for this.
...
the game must be won by one of
...
by 5 more minutes, unless both teams scored zero points (Points awarded for commentary are not considered in this decision). If after five
minutes there is still no winner, the team scoring the first points
during the extension will win.
