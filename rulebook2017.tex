...
\ang{45} steps. The orientation of the MPS Station is specified as
follows. Each MPS Station has a well-defined local right-handed
(x,y)-coordinate system: The x-axis is given by the conveyor pointing
from the output to the input. The origin is the center point of the
conveyor. Hence the

orientation of a MPS Station is equal \ang{0} if its local coordinate system is in
...
The orientations of the machines on the two halfs of the field are
mirrored along the field's y-axis. Thus, a machine on the magenta side
with an orientation of \ang{45} (respectively \ang{135}, \ang{225},
\ang{315}) is related to a machine on the cyan side with an
orientation of \ang{225} (respectively \ang{135}, \ang{225},
\ang{315}).
...
Therefore all CS and RS which are placed next to walls (zones: C-Z*1, C-Z*8, C-Z7*, M-Z*1, M-Z*8 and M-Z7*) are rotated in a way that the y-axis is facing away from the wall they are placed next to.
