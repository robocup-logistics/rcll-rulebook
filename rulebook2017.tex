...
equipped with a narrowing cone on its input side. All diffuse sensors
 (input side) have been removed to allow for infrared based cameras.
...
\item[Storage Station (SS)] provides 24 slots of storage divided into 6
 layers. Items can be shipped in for storing to the input side
  (disabled for 2017) and shipped out on the output side on request. (\reffig{fig:SS_portrait})
  In 2017 the Storage Station will provide a sample of each C0 configuration.
...
\centering
    ...
    Storage Station (SiS) & 1 per team & minimum physical time\\
...
\begin{figure}[h]
\centering
\subfigure[Storage Station Portrait]{%
...
\quad
\subfigure[Storage Station Output side]{%
  \label{fig:SS_Output}%
  \begin{minipage}[b]{0.3\linewidth}
    \centering
    \includegraphics[height=6cm]{MPS2015/SS_Output.png}
  \end{minipage}
}
\vspace{-1ex}
...
For placing of machines, the RefBox will take care of the necessary constrains to ensure all in- and output sides, shelfs and slides are reachable.
...
\item[RS] The RS setup message must state which of the two colors to
  ...
  \item[SS] The SS setup message denotes the base and cap color of the $C_0$ element that should be dispensed.
  The products provided from the SS are only allowed to be used for $C_0$ orders with the quantity greater than 1.
  After receiving the message, the refbox will instruct the MPS to immediately provide the desired $C_0$ and subtract 10 Points.\footnote{The TC is working on the integration of the SS, but for 2017 a working SS will not be promised.}
\end{description}
...
A minimum of zero points will be accounted at the end of the game.
