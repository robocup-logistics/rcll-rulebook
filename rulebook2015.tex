...
%Problems to include graphic  arguments (, angle=90, origin=c) not working properly 
  \centering
...
their unknown factory environment, i.e. identify where each of their six 
different MPS stations is located within the competition area.

The exploration phase starts with the announcement of the exploration 
game phase by the Referee Box. An exploration information message is sent, 
which assigns each elliptical zone to a team respecting that all MPS are 
placed within a zone belonging to the same team. Note that out of the 24 
zones only half of them are actually occupied by an MPS, the robots have 
to distinguish between occupied and free areas on their own.

Both teams have 3 minutes to explore the competition court. The task is to 
scan an AR tag of each MPS belonging to your team and detect the light of 
the signal lamp on top of the respective MPS. There are seven possible light 
signals with at least one LED switched on (no flashing lights).

Considering a mapping also send by the Referee Box at the start of the 
exploration phase, the robot can relate both the machine AR tag ID and 
the light state to a code which is then send back to the Referee Box for 
announcing the exploration of a machine. For example, if a robot detects 
red and green LED switched on at MPS with tag id 3, he looks up a code of 
e.g. 23 and announces that to the Referee Box. Therefore, the robot has to 
send a \emph{machine identified message} to the Referee Box. Please consult 
the RoboCup Logistics League Referee Box Integrator's Manual for a detailed 
definition of the message types. Note that the code is arbitrary just for 
the sake of the exploration phase and is not connected to the physical type 
of the machine.
  ...
  \subtable[Scoring scheme for the production phase]{
    \mytable{
      \begin{tabularx}{\linewidth}{p{7em}|X|p{2.4em}}
        ...
        Finish $C_0$ pre-cap & Mount the last ring of a $C_0$ product & $+0$ \\
        ...
        Just-in-time Production & The cap of a
        successfully delivered good has been mounted within the
        order time window& $+5$\\
