...
\usepackage{hyperref}
\hypersetup{
  pdftitle      = {The Logistics League sponsored by Festo Rulebook for 2015},
  pdfauthor     = {LLSF TC},
...
RFID-mounted machines, mark-ups, stocks of raw-material and delivery
zones. It is surrounded by boards of \SI{0.5}{\metre} height to reduce
object interference from outside the area.
...
gates features a black centered square of \SI{0.1 x 0.1}{\metre}
called delivery slot, which resides exactly below the RFID device.

Additionally to the delivery gates, 26 machines are placed within the
competition area: 24 machines representing the multi-staged production
...
pallet carrier can be identified by a unique number. The tournament
puck features a diameter of \SI{7.5}{\centi\metre} and is shown in
Figure~\ref{fig:puck}. Please consult Appendix~\ref{sec:data-carrier}
for further information of the RFID tag.
...
there are seven possible light signals with at least one LED switched
...
fixed. The machine type of each production machine will be randomized
prior to each match. The processing time of each machine type will be
determined in the same way, so the waiting time during a match will be
static for each machine of all machine types (e.g. all T1 could have 7
seconds processing time). The active delivery gate will also be
randomized prior to each match, but during a match the active gate can
switch. At the beginning, each team has the responsibility to place up
to 22 raw materials (pucks in $S_0$ state) in their input storage,
initially spread as shown in \reffig{fig:competition-area} or in
another fashion as chosen by the team. The pucks may only be touched
by a robot after the game has started. In particular, they may not be
re-positioned for better alignment if they have been pushed or moved
by a robot during the game.
...
products and machines. This includes position, orientation, and type
of each machine, as well as the different product variants that can be
produced.
...
multi-staged production processes can be repeated as long as enough
...
\begin{figure}
  \ffigbox{%
    ...
    \subfigure[$C_1$]{\label{fig:prod-compl-1}%
      \begin{tikzpicture}
        \path pic {base=red} ++(0,.55) pic {ring=blue} ++(0,.195) pic {cap=black};
      \end{tikzpicture}
    }
    \hfill
    \subfigure[$C_2$]{\label{fig:prod-compl-2}%
      \begin{tikzpicture}
        \path pic {base=silver} ++(0,.55) pic {ring=blue} ++(0,.55) pic {ring=green} ++(0,.195) pic {cap=black};
      \end{tikzpicture}
    }
    \hfill
...
\begin{figure}
  \ffigbox{%
    \centering
    \tikzstyle{M}=[rectangle, draw=blue!50, thick, fill=blue!20, minimum height=2em,
    ...
    ring according to \reffig{fig:prod-compl-1}.]{\label{fig:prod-chain-C1}%
      ...
      \begin{tikzpicture}[node distance=4em,level distance=1.6cm,sibling distance=5mm,draw=orange!80]
        ...
        \draw [every pic/.style={scale=0.3}] (cs-input) pic {any-base} ++(0,.06) pic {cap=black};
      \end{tikzpicture}
      \end{minipage}
    }
    ...
    \subfigure[Production of complexity $C_2$ with a two intermediate
    ring according to \reffig{fig:prod-compl-2}. Note that RS 2
    requires an additional BE (of any color) for the green ring.]{\label{fig:prod-chain-C2}%
      \begin{minipage}{1.0\linewidth}
        \centering
      \begin{tikzpicture}[node distance=4em,level distance=1.6cm,sibling distance=5mm,draw=orange!80]
        \node[M] (bs) {BS}  [thick]
    ...
    ring.]{\label{fig:prod-chain-C3}%
      ...
      \begin{tikzpicture}[node distance=4em,level distance=1.6cm,sibling distance=5mm,draw=orange!80]
        % \node[P] (cap) {Cap}  [thick]
        % child[grow=left] { node[M] (r1) {Yellow}
        %   child[grow=up,left,level distance=4em] { node [P,left=.8] {AE1} }
        %   child[grow=up,level distance=4em] { node [P] {AE2} }
        %   child[grow=left] { node[M] (r2) {Blue}
        %     child[grow=left] { node[M] (be) {BE} }
        %   }
        % };
        \node[M] (bs) {BS}  [thick]
        child[grow=right] { node[P] (be) {}
  ...
  \caption{These Production Chain Diagrams show the production steps
    of four example products
    (cf. \reffig{fig:production-complexities}). Blue boxes represent
    the machines that are used. RS 1 can mount yellow and blue rings
    and RS 2 can mount orange and green caps. Mounting an orange ring
    requires one additional BE (of any color). Similarly two
    additional BE are required for a yellow ring. Cap stations require
    a BE (of any color) with a cap of the matching required color.
    \emph{Note that this is a particular example. The actual
      production chains and requirements for additionale BEs are
      determined randomly by the referee box for each game}.}
  \label{fig:production-diagram}  
}
\vspace{-1mm}
\end{figure}
...
production step and delivery. Each order will require one or more
products of a specified product type to be delivered. The delivery
