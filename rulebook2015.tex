...
\begin{titlepage}
  ...
  \begin{center}
    ...
    \end{Large}
    \begin{tabular}{lll}
    \textbf{Christian Deppe}&Daniel Ewert&Nils Harder\\
    \textbf{S\"oren Jentzsch}&Nicolas Meier&\textbf{Tim Niemueller}\\
      Sebastian Reuter&\textbf{Alain Rohr}&\textbf{Wataru Uemura}\\
    \end{tabular}
    \vfill
    Revision Date: Friday, March 13\textsuperscript{\raisebox{-.2ex}{th}} 2015%DRAFT -- Work in Progress
  \end{center}
\end{titlepage}
\thispagestyle{empty}
\pagebreak
...
\subsection{Technical Committee 2015}
\label{sec:tc}
...
\subsection{Organizing Committee 2015}
\label{sec:oc}
...
unapproachable machines. However, the actual competition area will
change before the actual game starts. Thus, teams should focus on a
generic approach for production, allowing for dynamic adaptation of
machine positions and alignments.
...
conveyor belt input and output, shelves, and slides for additional
...
phase,\footnote{Note that in 2015 the BS and DS are assigned to fixed
...
individual light signal pattern randomly. The pattern consists of a
...
the green light will flash the green light for up to 3 seconds (it
...
performed. Once processing completes, the machine switches back to a
...
\item[CS] The CS setup message initiates either the retrieval or the
  mounting of a cap. For retrieving, a base from the shelf with an
  existing cap must be fed into the machine from which the cap is
  buffered. For mounting, a workpiece must be provided onto which the
  cap is mounted.
...
machine.\footnote{Note that canceling is not available in 2015.}
...
that are required for some colors. We therefore distinguish
\emph{color complexities} as $CC_0$, $CC_1$, and $CC_2$ depending on
whether zero, one, or two additional bases are required for a
color. The referee box ensures that there will always be some orders
for $C_1$ products where the ring color does not require additional
bases, i.e. with color complexity $CC_0$ -- but not necessarily all
$C_1$ orders have this constraint.
