...
software. %on the robot which is limited
...
specified to the referee before each game.  Only this team member will
be allowed to access the field area and only in case of a recent
...
\refsec{sec:production-communication}.
...
that are required for some colors. We distinguish \emph{color
  complexities} as $CC_0$, $CC_1$, and $CC_2$ depending on whether no,
one, or two additional bases are required for a color. The referee box
ensures that there will always be some orders for $C_1$ products where
the ring color does not require additional bases, i.e. with color
...
(cf.~\refsec{sec:production-communication}). This depends on the
