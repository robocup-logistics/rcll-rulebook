...
robots transport products in various states of manufacturing between 
different processing machines. 
Complete work orders require all related sub-assemblies of
product variants (cf.  \refsec{sec:prodportfolio}). Such
transformations can rely on more than one product as input to be
triggered.
The work flow is controlled by a referee box broadcasting information.
Final products are controlled by the delivery stations comparing to the 
pending jobs from the referee box broadcasting information
...
rectangular base shape of \SI{0,35 x 0,7}{\metre} with a height of about
\SI{1}{\metre} depending on machine type. Conveyor  The machine have individual application modules that
provide different functionallity.
All machines share a basic Layout: Trolley, conveyor belt and Signal unit are identical. The trolley also features identical positions for ALVAR tags. The trolley is movable by four wheels with  \SI{0,1}{\metre} clearance body has Both sides of the trolley are closed by plexiglass with a handle. This measures the total dimensions to \SI{0,35 x 0,7088}{\metre}. All physical interfaces (conveyor belt input and output, shelves and set up slides for ring stations are accessible at \SI{0,898}{\metre} height. Working space between guiding lanes is \SI{0,045}{\metre}. Set up lanes and shelves feature approximately the same space for handling and adjusting. If observed from the Output side of machines(where the signal unit is placed) the conveyor space opens up at half of the aluminium proflile (visible by abreak in the black plastic below), thus starting at \SI{0,35}{\metre}. Based on these shared properties, there are four kinds of machines:

\begin{figure}[h]
\centering
\subfigure[General Layout]{%
  \label{fig:MPS_Layout}%
...
\item[Base Station (BS)] acts as dispenser of base
  elements (cf.\refsec{sec:prodportfolio}). The application modules
...
\subfigure[Base Station Portrait]{%
  ...
  \begin{minipage}[b]{0.3\linewidth}
    \centering
    \includegraphics[height=7cm]{MPS2015/BS_portrait.png}
  \end{minipage}
} 
\quad
\subfigure[Base Station Output side]{%
  ...
  \begin{minipage}[b]{0.35\linewidth}
    \centering
    \includegraphics[height=7cm]{MPS2015/BS_Output.png}
  \end{minipage}
}
...
\item[Cap Station (CS)] mounts a cap as the final step in production
  on an intermediate product. The application module is a vacuum pick
  \& place module. A base element with a cap must be taken to the
  machine and is then unmounted and buffered in the machine. The cap
  is then mounted on the next intermediate product taken to the
  machine. There are two CS per team.
...
\subfigure[Cap Station Portrait]{%
  ...
  \begin{minipage}[b]{0.3\linewidth}
    \centering
    \includegraphics[height=7cm]{MPS2015/CS_portrait.png}
  \end{minipage}
} 
\quad
...
\caption{Cap Station}
\label{fig:CS-portrait}
\end{figure}

\item[Ring Station (RS)] mounts one colored ring out of two available
  colors on an intermediate product. Each RS has two vacuum pick \&
  place units as application modules with separate unique colors which
  are determined new for each game. There is an additionally pre-fill
  slide which is used for some colors (specified anew for each game)
  to add base elements. There are two RS per team.
...
\subfigure[Ring Station Portrait]{%
  ...
  \begin{minipage}[b]{0.3\linewidth}
    \centering
    \includegraphics[height=7cm]{MPS2015/RS_portrait.png}
  \end{minipage}
} 
\quad
\subfigure[Ring Station Output side]{%
  ...
  \begin{minipage}[b]{0.35\linewidth}
    \centering
    \includegraphics[height=7cm]{MPS2015/RS_Output.png}
  \end{minipage}
}
\caption{Cap Station}
\label{fig:RS-portrait}
\end{figure}

\item[Delivery Station (DS)] Accepts completed products. The stations
  contains three conveyor belts. The robot has to choose the proper
  one for a specific orer. The delivered products are verified by
  either the referees or an automated external vision system. There is
  one DS per team.

\begin{figure}[h]
\centering
\subfigure[Delivery Station Portrait]{%
  \label{fig:RS_portrait}%
  \begin{minipage}[b]{0.3\linewidth}
    \centering
    \includegraphics[height=7cm]{MPS2015/DS_portrait.jpg}
  \end{minipage}
}
\caption{Delivery Station}
\label{fig:DS-portrait}
\end{figure}
...
(Figure \ref{fig:competition-area}) to a team number with the start of the 
...
To enable the production process. Afterwards the machine will turn back to showing green lights.
...
%%This should be partly covered by RefBox MMI
...
Once an order is completed, the product is has to be shipped to Delivery Station(DS). After the physical interface, the Robot has to command which of the three lanes is to be used to store the product(which truck to load it on). More than one lane can be correct, depending on the relevant order.
