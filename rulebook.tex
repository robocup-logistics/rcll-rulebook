...
The storage status of each position is periodically sent to each team,
...
\begin{figure}[ht]
  \centering
    ...
    \caption{The bottom-most shelf of the SS. Position $(x,y)$ consists of
      shelf
    number $x$ and slot number $y$. Shelves are numbered in ascending order from bottom to top.}
    \label{fig:ss-shelf}
\end{figure}
...
Retrieving, storing and relocating workpieces induce costs per operation.
...
\begin{longtable}{p{\dimexpr .30\textwidth-2\tabcolsep
      ...
      %  }
        ...
        \\
        Relocate at SS
        & A storage/retrieval request to an inaccessible slot requires
        relocation of a blocked workpiece
        & $0$
        \\
        Upkeep at SS
        & A workpiece stored at time $t$ causes costs $f(x)$ at
        ${t+x\cdot\SI{60}{\second}}$:
        \begin{tikzpicture}[baseline={(current bounding box.north)}]
          \node[align=left] (conditions) {
           if $x < 3$\\
           if $3\leq x \leq 7$\\
           else
            };
          \node[align=right,inner sep=0pt] at ([xshift=-1cm]conditions.west)
               (costs) {
                        $0$ \\
                        $-1$\\
                        $0$
          ...
          \draw[decoration={brace,mirror,raise=5pt},decorate]
            (costs.north west) -- node[left=6pt] {$f(x)=$}
               (costs.south west);
       \end{tikzpicture}
        & $\leq 0$
        \\
        Total costs SS
        & Combined costs from SS usage may not exceed this value
        & $30$
        \\
        Delivery
