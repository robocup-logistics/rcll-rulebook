...
partner Festo provides insights into future factory concepts to help
 ...
 to develop the \ac{RCLL} further. Smart Factory environments require new
 ...
 infrastructure, the \ac{RCLL} aims to provide concepts for smart production
 logistics.
 ...
 production steps.  Additionally, the communication with the \ac{MPS} stations got
 ...
 While some of the complexity of the regular \ac{RCLL} game could not be projected
...
Then the \ac{refbox} switches to the production phase, during which the \ac{refbox}
...
advancements. Current members of the \ac{TC} are (in alphabetical order):\\[.5em]
...
\subsection{Machines}
\label{sec:machines}
Machines are based on the \acf{MPS}\footnote{For
  more information see
  \url{http://www.robocup-logistics.org/links/festo-mps}.} by Festo
Didactic SE\@. The \ac{MPS} provides a number of stations which
...
% TODO: mention optional workpiece trackers
...
The zones and rotations for the \ac{MPS} will be randomly chosen by the \ac{refbox}.
Each game will have a new randomly generated field layout, the referees will
place the \ac{MPS} in the middle of the zone with given rotation at there best effort
but errors of the positioning should be expected.

A \ac{MPS} can be placed in any zones except those at the entrance of the
insertion area (the light red areas in \reffig{fig:competition-area} and
\reffig{fig:competition-area-challenges}).
These zones are needed to enter the field. Each \ac{MPS} has one out of 8 possible
...
types \ac{BS}, \ac{DS}, and \ac{SS} (even when adjacent to the wall) are mirrored according to
\reftab{tab:mirror-a}. Second, \ac{CS} and \ac{RS} stations must be handled slightly
...
problem, the \ac{RS} and \ac{CS} machines adjacent to a wall will always be placed such
that the shelf or slide is on the farther end with respect to the wall. For
example, in \reffig{fig:competition-area}, the magenta \ac{CS} in M-Z77 with a
rotation of \ang{270}. Were this setup mirrored, the shelf on the cyan \ac{CS} in
...
\begin{figure}[tb]
    \centering
    \subfigure[
      For rotations with \ang{0}, \ang{90}, \ang{180} or \ang{270}. \ac{BS}, \ac{CS}, \ac{RS},
      and \ac{SS} block the zones close to the input as well as the output. \ac{DS} blocks
    ...
    \quad
    \subfigure[
      For rotations with  \ang{45}, \ang{135}, \ang{225} and \ang{315}. \ac{BS}, \ac{CS},
      \ac{RS}, and \ac{SS} block the zones close to the input as well as the output. \ac{DS}
    ...
    \caption{
      Blocked zones regarding the \ac{MPS} rotation (red zones must not be used as
      position for other machines and must be located inside the competition
      area).
    }
    \label{fig:zone-mps-blocked}
\end{figure}
For placing of machines, the \ac{refbox} will take care of the necessary constraints
...
(\reffig{fig:zone-mps-rotation-odd}). No \ac{MPS} nor blocked zones for another \ac{MPS}
may be placed in a blocked zone. All blocked zones of any \ac{MPS} must be located
...
of the box, such that it is accessible from the front on stations replicating a
		\ac{RS}
\end{itemize}
Models for a conveyor belt, shelf and slides can be found in the \ac{RCLL} rulebook
repository\footnote{\url{https://github.com/robocup-logistics/rcll-rulebook/tree/master/mock_up_models}}. % chktex ignore-long-line

The building materials for the models must be opaque, but may have any color.

\section{Game Play}
There are different game modes within the \ac{RCLL} (see \refsec{sec:tournament-main}
and \refsec{sec:tournament-challenges}).
At the beginning of a match, the \ac{refbox} must be configured according to the
game mode specification.
The \ac{refbox} then guides the match utilizing different phases, which are outlined
...
During the setup phase, the \ac{refbox} announces the layout of the competition area,
...
allowed. The robot must react to the \ac{refbox} game state messages
to start the game play autonomously.
The \ac{refbox} will control the setup period and automatically end it
after 5 minutes.
...
environment and report their machines to the \ac{refbox} in order to score
points.
The report about an \ac{MPS} may be split
in two parts, the position and type of a machine, and its orientation.
Concretely, the \ac{refbox} expects a \emph{machine identification
...
filled to account for the two parts. Please consult the \ac{refbox}
Integrator's Manual~\cite{RefBoxIntManual} for a detailed definition
of the message types. Reports are only accepted for \ac{MPS} of the
respective team over the private encrypted team channel. Only the
first information for each \ac{MPS} is processed, hence per machine only one attempt
...
The signal light of the \ac{MPS} will show a yellow light until a report regarding
this machine is sent.  If the wrong \ac{MPS} zone is sent, the light will start red
flashing.  If only the \ac{MPS} zone is sent, the green light will be turn on.  If
...
\emph{additional bases} to a \ac{RS} first in order to get a ring. We
...
\begin{figure}[p]
    ...
    from the \ac{BS} (brown arrow) are used in this example to fulfill the payments.
...
If a product from the input is required for the processing step (\ac{DS}, \ac{CS}, \ac{RS} and
a storage operation at the \ac{SS}), then the conveyor belt is started
...
After performing an operation at the \ac{BS}, \ac{CS}, \ac{RS} or the \ac{SS} (in case of a
retrieval) the conveyor belt is started. If no workpiece reaches the destination
point (indicated through the output belt sensor, or in case of the \ac{BS} either
the input or output belt sensor) within \SI{90}{\second}, the machine goes in a
temporary out-of-order state (cf.~\refsec{sec:broken-machine}).
...
The \ac{DS} prepare message denotes the order ID of the product that is delivered.
The \ac{DS} will consume any workpiece provided, but points can only be scored if the
\ac{DS} has been properly prepared.
...
The \ac{SS} prepare message denotes the storage place (shelf and slot number) and
...
$(x,y-1)$ is occupied causes the \ac{SS} to relocate the product at $(x,y-1)$ to a
...
Note the costs for using the \ac{SS} outlined in \refsec{sec:production-scoring}.
...
An \ac{MPS} may be reset by a team at any time by a dedicated msg (see
\ac{refbox} Integrator's Manual~\cite{RefBoxIntManual}). This causes
the \ac{MPS} to go into the broken state as well.
...
Depending on the played format, The \ac{refbox} may take down machines
randomly out of the pool containing the \ac{RS} and \ac{CS}.
...
\rowcolors{1}{gray!40!white}{gray!30!white}
...
rings, prepared caps on shelves, and $C_0$ in the \ac{SS}) before and during matches.
...
For example, teams can fill any number of bases into a \ac{BS} or are free to choose
the color of caps for a \ac{CS}, however teams are only allowed to fill the \ac{CS} if the
whole shelf is empty and then it has to be filled completely.  The color
assignment of bases on the \ac{BS} and \ac{SS} must be obeyed, as otherwise they will
deliver the wrong bases. Similarly, ring color assignments at the \ac{RS} must be
...
\SI{14 x 8}{\metre} and each team has one \ac{BS}, \ac{SS} and \ac{DS} as well as %chktex 29
two \ac{CS} and two \ac{RS} available, so a total of 14 machines are placed on the field.
...
for the \ac{refbox} to properly register the zones as visited. The robot may move
...
shown in \reffig{fig:competition-area-challenges}, where the \ac{BS} is placed at
M-Z12, CS1 is placed at M-Z53 and \ac{RS}1 is placed at M-Z25.
...
% TODO: actually write the wiki article
...
A robot moving a workpiece of the opposing team at their \ac{MPS} is considered a
misbehaving robot.
