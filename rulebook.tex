...
\label{sec:intro}
The future of industrial production is smarter and more flexible 
production. Manufacturing industries are on the brink of 
...
Then the \ac{refbox} switches to the production phase, during which the
...
\emph{Vincent Coelen}, Ecole polytechnique universitaire de Lille, Lille, France\\
...
Each \ac{MPS} shares the same basic layout as highlighte in Figure \ref{fig:MPS-Layout}. 
A movable rectangular trolley makes up the the base of the machine. 
A conveyer belt connecting the two short sides of
the machine is the main interaction point for robots. Three colored signal lights 
are used to visually indicate the current state of the machine. Each machine type has
additional individual application modules which provide the different functionalities.
...
The stations have arectangular base shape of \SI{0,35 x 0,7}{\metre} with a height of % chktex 29
about \SI{1}{\metre} depending on machine type. A machine is movable by four
 wheels with \SI{0.1}{\metre} clearance. Both narrow sides of the trolley are
...
A \ac{MPS} can generally  be placed in any zones except those at the entrance of the
...
possible orientations, starting at \ang{0} with steps of \ang{45}. To denote
...
Once the setup phase ends, no more interference (physically or remotely) with the 
robot is allowed. The robot(s) must react to the \ac{refbox} game state messages
and start the game play autonomously.
...
During the exploration phase the robots of each team have \SI{3}{\minute} to roam the
environment and report their (and only their) machines to the \ac{refbox}. Correctly 
reported machines will be rewarded with points. 
Reports are issued to the refbox using a \emph{MachineReport message}. For each machine
,identified by its name, both its zone and its orientation (in degrees) can be explored. Both 
properties can be submitted at the same time in one message or be split up into
multiple messages. The refbox accepts only the first submitted value for each property. For 
a detailled description of the message please refer to the \ac{refbox} wiki.
...
\begin{description}
\item[Base] The base is the lowermost of each workpiece.
...
the ring. We therefore distinguish \emph{color complexities} as $CC_0$, $CC_1$, and
...
were required but which have not been paid for, the machine will
...
machines at random. If two teams play at the same time, then the same machines for
...
by the duration of the time spent offline). The downtime is indicated by a steady red
...
during matches.
Each team has to designate one team-member as a \textit{``field-opreator''} who
must be specified to the corresponding referees ahead of each
game. Only this team member will be allowed to access the field area in case of a 
refill procedure. The field-operator must not obstruct other
...
The field-operator may restock only caps if transparent base elements have been put
