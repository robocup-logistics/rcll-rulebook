...
divided into two phases: During a setup phase (see \refsec{sec:setup-phase})
...
announces the tasks that the robots must fulfill automatically.
...
\begin{itemize}
  \item Production logistic scenarios where two teams compete against
    ...
    dispatched orders. This game mode is referred to as the main track.
  \item Individual challenges carried on a field, where only one team competes
   ...
   This game mode is referred to as the challenge track.
\end{itemize}
Depending on the task (see \refsec{sec:game} and \refsec{sec:challenges},
...
Different machines are placed within the area, he constraints for
...
into a running game. The usage of these areas is restricted to the purposes,
...
\reffig{fig:competition-area-challenges}, it spans a \SI{5 x 5}{\metre} large
arena with 25 square zones, where 3 machines are placed. %chktex 29
...
For symmetrical fields, the machines will be placed mostly on the primary half
...
They markers are available for printing on the RoboCup Logistics website.
...
% TODO add game format section
There are different game modes within the RCLL (see \refsec{sec:formats}).
...
The competing team(s) may use the setup phase to start up their robots.
...
environment and report their machines to the referee box.
...
% TODO: move this
%The referee box ensures that there will always
...
\begin{figure}[p]
    ...
    A cap-less carrier, a capcarrier and a fresh base from the BS are
    used here to fulfill the payments.
...
The available challenges are defined in \refsec{sec:challenges}.
...
%TODO: this is likely unnecessary with the new refbox config management
...
The following situations may cause a robot to be labeled misbehaving:
\begin{itemize}
