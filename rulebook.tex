...
Each team is assigned an IPv4 address range within the network of the refbox
(172.26.x.x/16) by the \ac{TC}.
The \ac{TC} provides an updated router specification in the network
documentation. A comparable router must be supplied on-site at an event.
Exact specifications of the network might need to be adapted based on local
event circumstances. The local organizers or TC must provide these
specifications before the qualification day of a tournament.
