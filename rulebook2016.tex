...
\item[CS] The CS setup message initiates either the retrieval or the
    mounting of a cap. For retrieving a preapred base (with a cap mounted on top) 
    from the machine's shelf must be fed into the machine. All Base Elements prepared on the shelf are special colored workpieces that can only be used for preparing RS or recycling. The machine will take the cap off the base and buffer it, releasing the decapped base to the output side. For mounting, a base(with up to three rings) must be provided onto which the cap is mounted.
...
\begin{description}
\item[Base] The base is the lowermost element in each production. All bases are labelled by a horizontally revolving, white glossy tag covering about 50 percent of the bases height (from the centre) containing a white reference area as well as an identifying barcode. %%ADDPICTURE%%
  ...
  the colors red, black, and silver. There is a fourth base type: Transparent, which is the only base that must be used to prepare shelves mentioned in the CS part (cf.~\refsec{sec:production-machines-production}). These bases cannot be used as workpiece for production. Transparent bases can be used to provide additional bases required by RS, stored on a friendly shelf or recycled at DS. \footnote{Note that \textbf{in
...
rings, and prepared caps on shelves) before and during matches. Each team has to designate one team-member as a
\textit{"replenisher"} who must be specified to the corresponding
"field half/team" referee ahead of each game.  Only this team member
...
enter the competition-area without asking the referee. Shelves have to be (re)filled in all three reachable slots. %%%NotesInOfficeLetMeCheckIfWeDecidedOnThis%%%
  ...
  \centering
    ...
    \subtable[Scoring scheme for the exploration phase]{%
      ...
      \mytable{\begin{tabularx}{\linewidth}{p{6em}|p{4em}|X|p{4em}}
          \multicolumn{1}{l}{Report type} &\multicolumn{1}{l}{Correct} & \multicolumn{1}{l}{Description} & \multicolumn{1}{l}{Points}\\\hline
