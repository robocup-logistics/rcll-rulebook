...
\begin{titlepage}
  ...
  \begin{center}
    ...
    \end{tabular}
    \vfill
    Revision Date: DRAFT -- Work in Progress %Wednesday, July 1\textsuperscript{\raisebox{-.2ex}{st}} 2015
  \end{center}
\end{titlepage}
\thispagestyle{empty}
\pagebreak
...
and less frequently and there were times with no orders. In 2015 we
introduced actual physical processing machines based on the Festo
...
height.\footnote{Note however, that due to small variances and
  ...
  working height of some or all parts of a station.} Working space between guiding lanes
is \SI{4.5}{\centi\metre}. Setup lanes and shelves feature
approximately the same space for handling and adjusting.
...
vertical distance (between tag and conveyer belt) will be about 
...
After a successful report, the respective MPS will show a blinking
green signal indicating visually for spectators that the MPS was
indeed explored successfully. For a wrong report, the station will
...
\item[CS] The CS setup message initiates either the retrieval or the
    mounting of a cap. For retrieving, a base (with a cap mounted on top) 
    from the machine's shelf must be fed into the machine. The machine will 
    then take the cap off the base and buffer it. For mounting, a workpiece 
    must be provided onto which the cap is mounted.
...
\item[Base] The base is the lowermost element in each production.
  ...
  the colors red, black, and silver.\footnote{Note that \textbf{in
      2015}, \textbf{orders} from the referee box \textbf{will only
      mention red bases} as to simplify the task and the transition
    from the 2014 game. This will change in 2016, but the colors may
    be changed for better visibility. This is subject to discussion of
    the league members and the TC.}
...
for a successful exploration to the referee box.
The detection of one MPS can be split in two parts, first the detection of the machines position and type and then the detection of the machines light signal.
Concretely, the referee box expects a \emph{machine identification message} with a
tuple consisting of zone, machine name (as encoded in the markers),
light pattern, and type string.
For the two different stages of the detection of one MPS, this message can just be party filled.
Please consult the referee box
...
first report for each MPS is processed, all additional ones will be
silently ignored. The scoring is shown in
...
after the time has passed or immediately after both teams have each
reported their respective set of 6 MPS.
...
game with a delivery time window in the last third.
...
\textit{"replenisher"} who must be specified to the corresponding
"field half/team" referee before each game.  Only this team member
will be allowed to access the field area and only in case of a recent
refill procedure.  The replenisher must not obstruct other robots and
should interfere as little as possible.  The machines can only be
refilled when a magazine is empty. In this case the field-operator may
enter the competition-area without asking the referee.
...
\begin{table}
  \centering
    ...
    \subtable[Scoring scheme for the exploration phase]{%
      ...
      \mytable{\begin{tabularx}{\linewidth}{p{6em}|p{4em}|X|p{4em}}
          \multicolumn{1}{l}{Report type} &\multicolumn{1}{l}{Correctly} & \multicolumn{1}{l}{Description} & \multicolumn{1}{l}{Points}\\\hline
          \multirow{2}{*}{Position} & yes & Correctly determine a machine type of your team and report it successfully to the refbox &	+3\\
    & no & Wrongly reported machine type &  -2\\
    \multirow{2}{*}{Light} & yes & Correctly determine a machine light pattern of your team and report it successfully to the refbox & +5\\
    & no & Wrongly reported machine light pattern & -4\\\hline
    \multicolumn{2}{l|}{Round Total} & A maximum of 48 points can be achieved by correctly
  ...
  \subtable[Scoring scheme for the production phase]{
    ...
    \mytable{
      \begin{tabularx}{\linewidth}{p{7em}|X|p{4em}}
        ...
        Finish $CC_1$ step & Finish the work order for a color requiring one additional base & $+5$\\
        Finish $CC_2$ step  & Finish the work order for a color requiring two additional bases & $+10$ \\
        ...
        Obstruction Penalty & Deliver a workpiece to a machine, which belongs to the opposing team & $-2$ \\
      \end{tabularx}
    }
  }
...
case, the workpiece becomes junk and cannot be used afterwards.
...
\paragraph{Playoffs.~}
At the playoff stage, the scoring scheme will be different. Each team
in this phase directly competes with an opposing team. To allow a fair
comparison, teams gain a different game score. These scores are then
summed up to form a ranking. We now describe this additional scoring
scheme.

For the \emph{game score}, the winning team gets 3 ranking points, the
loosing team gets 0. A team wins if it achieves more in-game points
(cf. Sections~\ref{sec:exploration-phase} and
\ref{sec:production-phase}) in the game than the other team. In the
case of a draw (with or without overtime,
cf.~\refsec{sec:production-phase}). If a non-zero draw is not
resolved, both teams get 1 ranking point. If both team end with a zero
score, each team gets 0 ranking points.

The \emph{overall playoffs ranking} is determined by the sum of the
ranking scores of all games per team, highest first. If two teams have
the same score, the overall total in-game points are summarized and
the team with more game points ranks higher. If there still is a draw,
the direct comparison of the games of the two teams is used to break
the tie (decided by higher in-game points summed over all games of the
two teams during the playoffs). If this still is unsuccessful, a coin
toss determines the higher ranked team.
