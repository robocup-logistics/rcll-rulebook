...
tackling this problem of flexible and efficient logistics at a
...
autonomy. Festo acts as the industrial partner for this league
providing feedback and advice regarding the industrial grounding of
the league and the basic elements like robots and production machines.
...
To simplify the delivery on the belt, each input side will be equipped with a 
narrowing cone (\reffig{fig:narrow-cone}). The exact dimensions are specified 
in the near future.
\begin{figure}
...
\item[Cap Station (CS)] mounts a cap as the final step in production
  ...
  is a vacuum pick \& place module. There is a slide to store one cap piece. 
  At the beginning this slide is empty and has to be filled in the following way.
  A base element with a cap must be
  taken to the machine and is then unmounted and buffered in the
  slide. The cap is then mounted on the next intermediate product
  taken to the machine. At the beginning the cap slide is empty and has to be filled. 
  It can hold only one cap at a time. There are two CS per team.
...
\item[CS] The CS setup message initiates either the retrieval or the
  ...
  machine. 
  All Base Elements prepared on the shelf are special colored
  workpieces that can only be used for preparing RS or reuse by repositioning
  on the shelf. The
  machine will take the cap off the base and buffer it on the slide, releasing the
  decapped base to the output side. For mounting, a base(with up to
  three rings) must be provided onto which the cap is mounted.
...
\begin{description}
\item[Base] The base is the lowermost element in each production. All
  bases are labeled by a horizontally revolving, white glossy tag
  covering about 50 percent of the bases height (from the centre)
  containing a white reference area as well as an identifying
  barcode. Each base is equipped with a unique ID which will be scanned at each station. The Referee Box will use Barcodes to track assembly processes, also awarding points.  %%ADDPICTURE%%
...
One standing order with the complexity of $C_0$ will be placed at the beginning of the production phase and can be delivered until the game finishes, however just one product of this type can be delivered for this order.
For this standing order the base color will be red and therefore not be randomized, this however does not change the randomization of the product cap.
...
slots. It is allowed to restock only the caps if the transparent base elements
have been put back into the shelf.
%%%NotesInOfficeLetMeCheckIfWeDecidedOnThis%%%
